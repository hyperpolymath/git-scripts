...
|Property-based (P2P) |PASS |`+test/script_manager_test.exs+`
(StreamData) |5
...
* `+gossamer+` git dep temporarily commented out (no mix.exs in the repo
root).
* `+web_interface.ex+` stubbed to compile without `+Gossamer.Web+`
macro.
